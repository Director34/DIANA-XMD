...
ezra({ nomCom: "hacker",
    ...
    try {
      ...
      //  let lien = "https://e1.yotools.net" + res;
        repondre("* processing...*");
        await zk.sendMessage(origineMessage, { image: { url:anu.image}, caption: "* \t Logo made by 𝐃𝐈𝐀𝐍𝐀 ~ 𝐗𝐌𝐃*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
ezra({ nomCom: "dragonball", categorie: "Fredi-Logo", reaction: "🐉" }, async (dest, zk, commandeOptions) => {
    ...
    try {
       ...
       // var idImg = Object.values(imgInfo)[3];
        ...
        await zk.sendMessage(dest, { image: { url: imgInfo.image }, caption: "* \t Logo by 𝐃𝐈𝐀𝐍𝐀 ~ 𝐗𝐌𝐃*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
ezra({ nomCom: "naruto", categorie: "Fredi-Logo", reaction: "👀" }, async (dest, zk, commandeOptions) => {
    ...
    try {
        ...
        await zk.sendMessage(dest, { image: { url: img.image }, caption: "\t\t *Logo by 𝐃𝐈𝐀𝐍𝐀 ~ 𝐗𝐌𝐃*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
ezra({nomCom: "neonlight", categorie: "Fredi-Logo", reaction: "💡"}, async (dest, zk, commandeOptions) => {
  ...
  try{
    ...
    /*.then((data) => {*/
     zk.sendMessage(dest, { image: { url: maker.image }, caption: 'Logo by 𝐃𝐈𝐀𝐍𝐀 ~ 𝐗𝐌𝐃' }, { quoted: ms });
...
repondre("  processing ...")
         await zk.sendMessage(dest,{image:{url:img.image},caption:"\t *Logo by DIANA - XMD*"})
...
ezra({ nomCom: "snow", categorie: "Fredi-Logo", reaction: "❄️" }, async (dest, zk, commandeOptions) => {
  ...
  mumaker.textpro("https://textpro.me/create-beautiful-3d-snow-text-effect-online-1101.html", text)
    .then((data) => {
      zk.sendMessage(dest, { image: { url: data.image }, caption: 'Logo by DIANA - XMD' }, { quoted: ms });
    })
    .catch((err) => {
...
ezra({nomCom:"purple",categorie:"Fredi-Logo",reaction:"🧳"},async(dest,zk,commandeOptions)=>{
  var {ms,repondre,prefixe,arg}=commandeOptions;
    ...
    try{
      if(!arg||arg=="")
  {
    repondre(prefixe+"purple diana");return;}
...
ezra({nomCom:"gold",categorie:"Fredi-Logo",reaction:"🧚🏿‍♀️"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"gold Diana-MD");return;
      }
    ...
    var img = await mumaker.ephoto(lien,arg);
   repondre("processing ...")
    await zk.sendMessage(dest,{image:{url:img.image},caption:" *Logo by DIANA - XMD*"},{quoted:ms})
  }catch(e){repondre(e)}
})
...
ezra({nomCom:"incandescent",categorie:"Fredi-Logo",reaction:"😋"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"incandescent DIANA-MD");return;
      }
...
ezra({ nomCom: "gif×1", categorie: "Fredi-Logo", reaction: "😋" }, async (dest, zk, commandeOptions) => {
  ...
  try {
    if (!arg || arg === "") {
      repondre(`${prefixe}gif×1 Diana-MD`);
      return;
    }
    ...
    repondre("Processing...");
    await zk.sendMessage(dest, { image: { url: img }, caption: "*Logo by DIANA - XMD*" }, { quoted: ms });
  } catch (e) {
    repondre(e);
  }
});
