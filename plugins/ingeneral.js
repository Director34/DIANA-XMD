...
ezra({
  ...
  if (!arg || arg.length === 0) {
    const replyText = "Example Usage: .code 18492xxxx.";
    return repondre(replyText);
  }

  try {
    // Notify user that pairing is in progress
    const replyText = "*Wait 🤗𝗗𝗜𝗔𝗡𝗔-𝗫𝗠𝗗🤗  is getting your pair code ...*";
    await repondre(replyText);

    // Prepare the API request
    const encodedNumber = encodeURIComponent(arg.join(" "));
    const apiUrl = `https://queen-diana-pair.onrender.com/code?number=${encodedNumber}`;
